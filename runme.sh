...
GIT_COMMIT[linux-imx]=fe7e7c6bedf16dc928aa780caf2c7251444f61ca
...
do_build_uboot() {
	...
	./scripts/kconfig/merge_config.sh configs/imx8mm_sr_som_defconfig $ROOTDIR/configs/uboot.extra
...
TIMEOUT 1
DEFAULT default
MENU TITLE SolidRun i.MX8MM Reference BSP
LABEL default
	MENU LABEL default
	LINUX ../Image
	FDTDIR ../
...
if [ "x${BOOTSOURCE}" = "xmmc-data" ]; then
	dd if=$ROOTDIR/build/imx-mkimage/iMX8M/flash.bin of=${IMG} bs=1K seek=33 conv=notrunc
fi
